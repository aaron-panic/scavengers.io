...
def update_request(req_id, status=None, message=None):
    conn = None
    try:
        conn = get_connection('admin_bot')
        execute_procedure(conn, 'sp_admin_update_request', [req_id, status, message], commit=True)
    except Error:
        raise
    finally:
        if conn and conn.is_connected():
...
def delete_request(req_id):
    ...
    try:
        conn = get_connection('admin_bot')
        execute_procedure(conn, 'sp_admin_delete_request', [req_id], commit=True)
    except Error:
        raise
    finally:
        if conn and conn.is_connected():
